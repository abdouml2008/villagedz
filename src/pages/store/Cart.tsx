                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-2">
                      {priceInfo.hasDiscount ? (
                        <>
                          <span className="text-primary font-bold">{priceInfo.discountedPrice.toFixed(0)} دج</span>
                          <span className="text-muted-foreground line-through text-sm">{priceInfo.originalPrice.toFixed(0)} دج</span>
                          <span className="bg-green-500/10 text-green-600 text-xs px-2 py-0.5 rounded-full">-{priceInfo.discountPercentage}%</span>
                        </>
                      ) : (
                        <span className="text-primary font-bold">{priceInfo.originalPrice.toFixed(0)} دج</span>
                      )}
                    </div>
                    {item.product.discount_quantity && item.product.discount_percentage && !priceInfo.hasDiscount && (
                      <p className="text-xs text-green-600 mt-1">
                        ...
                        اشترِ {item.product.discount_quantity} قطع أو أكثر واحصل على خصم {item.product.discount_percentage}%
                      </p>
                    )}
          ...
          <div className="bg-card rounded-xl p-6 shadow-village-md border border-border h-fit sticky top-24">
            ...
            <div className="space-y-2 mb-4">
              <div className="flex justify-between">
                ...
                <span>{(totalPrice + totalDiscount).toFixed(0)} دج</span>
              </div>
              {totalDiscount > 0 && (
                <div className="flex justify-between text-green-600">
                  <span>الخصم</span>
                  <span>-{totalDiscount.toFixed(0)} دج</span>
                </div>
              )}
              <div className="flex justify-between">
            ...
            <div className="border-t border-border pt-4 mb-6">
              <div className="flex justify-between text-xl font-bold">
                <span>الإجمالي</span>
                <span className="text-primary">{totalPrice.toFixed(0)} دج</span>
              </div>
            </div>
            <Link to="/checkout">
